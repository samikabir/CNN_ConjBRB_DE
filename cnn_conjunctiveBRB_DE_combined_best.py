...
def deRuleBase(s,c,best):
    ...
    temp_deConsequentBeliefDegree = [best[0], best[1], best[2], best[3], best[4], best[5], best[6], best[7], best[8], best[9], best[10], best[11], best[12], best[13], best[14], best[15], best[16], best[17], best[18], best[19], best[20], best[21], best[22], best[23], best[24], best[25], best[26]]
    for u in range(27):   
    ...
    print("Inside deRuleBase(best) relativeWeight1 ",best[27],"relativeWeight2 ",best[28])  
     
    H_PM = 0 + (best[38] * 499.4)     #500.4     
    M_PM = 0 + (best[39] * 499.4)     #35.5
    L_PM = 0 + (best[40] * 499.4)     #0    
    transformInput1(s,H_PM,M_PM,L_PM)      
    transformInput2(c)    
    calculateMatchingDegreeBrbCnn(best[27], best[28], best[29], best[30], best[31], best[32], best[33], best[34], best[35], best[36], best[37]) 
    showActivationWeight()  
    updateBeliefDegree()        
    deResult = aggregateER_BrbCnn()   
    fo = open("de_best.txt", "w") 
     
    for w_cbd in range(27):
        s_cbd = str(consequentBeliefDegree[w_cbd])   
        fo.write(s_cbd)  
        fo.write("\n") 
   
    for w_cmd in range(27, 38):    
